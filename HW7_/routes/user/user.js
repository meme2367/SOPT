...
router.get('/test', (req, res)=>{
    try{
        console.log(jwt.verify(req.headers.authorization, secretKey));
        ...
        if (err.message === 'jwt expired') {
            ...
            res.status(200).send(util.successFalse(statusCode.UNAUTHORIZED, resMessage.EXPRIED_TOKEN));

        } else if (err.message === 'invalid token') {
            console.log('invalid token');
            res.status(200).send(util.successFalse(statusCode.UNAUTHORIZED, resMessage.INVALID_TOKEN));
